                ...
                <div class="row">
                    <div class="col-sm-12">
                        <ul class="nav float-left ml-2">
                            <li><a href="{{ url('/dataruangan/create') }}" class="btn btn-info btn mb-2 ml-1"><i
                                        class="fas fa-plus"></i> Tambah Data</a>
                            </li>
                            <li><a href="{{ url('/dataruangan/exportexcel') }}" class="btn btn-success btn mb-2 ml-1"><i
                                        class="fas fa-file-excel"></i> Export
                                    Excel</a></li>
                            <li><a href="{{ url('/dataruangan/exportpdf') }}" class="btn btn-danger btn mb-2 ml-1"><i
                                        class="fas fa-file-pdf"></i> Export
                                    PDF</a></li>
                            <li>
                                <form action="{{ url('dataruangan')}}" method="GET">
                            ...
                            <div class="table-responsive">
                                <table class="table custom-table datatable">
                                    ...
                                    <tbody>
                                        @if ($data_ruangan->count() > 0)
                                            @foreach ($data_ruangan as $index => $row)
                                                <tr>
                                                    <th scope="row">{{ $index + $data_ruangan->firstItem() }}</th>
                                                    ...
                                                    <td>
                                                        <a href="{{ url('/dataruangan/edit/' . $row->id) }}"
                                                            class="btn btn-warning btn-sm"><i class="fas fa-edit"></i></a>
                                                        <a href="{{ url('/dataruangan/delete/' . $row->id) }}"
                                ...
                                <div class="float-right">
                                    <div class="text-right">
                                        <small>Ditampilkan {{ $data_ruangan->firstItem() }} -
                                            {{ $data_ruangan->lastItem() }} dari {{ $data_ruangan->total() }} Data</small>
                                        {{ $data_ruangan->links() }}
                                    </div>
                                </div>
                            </div>
